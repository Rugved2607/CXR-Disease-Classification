...
# FIX 1: Assign target_layer AFTER model is on device, and instantiate GradCAM ONCE
target_layer = model.features[-1]
cam = GradCAM(model=model, target_layers=[target_layer])


def add_caption(img_np: np.ndarray, disease: str, confidence: float) -> Image.Image:
    img_pil = Image.fromarray(img_np)
    new_img = Image.new('RGB', (img_pil.width, img_pil.height + 30), (255, 255, 255))
    new_img.paste(img_pil, (0, 0))
    draw = ImageDraw.Draw(new_img)
...
def predict(image):
    ...
    # FIX 2: Use no_grad ONLY for the probability pass — GradCAM needs gradients
    with torch.no_grad():
        output = model(input_tensor)
        probs = torch.sigmoid(output).cpu().numpy()[0]
    ...
    # FIX 3: GradCAM computed OUTSIDE no_grad so gradients flow correctly
    gradcam_images = []
    for disease_name, confidence in detected:
        class_idx = all_classes.index(disease_name)
        targets = [ClassifierOutputTarget(class_idx)]
        grayscale_cam = cam(input_tensor=input_tensor, targets=targets)[0]
        visualization = show_cam_on_image(img_np, grayscale_cam, use_rgb=True)
        captioned = add_caption(visualization, disease_name, float(confidence))
        gradcam_images.append(captioned)
...
with gr.Blocks(title="Chest X-Ray Classifier") as demo:
    gr.Markdown("# <h1 align='center'>Chest X-Ray Disease Classifier</h1>")
    gr.Markdown(
        "<h4 align='center'>DenseNet-121 · NIH CXR8 · 112,120 images · "
        "Mean AUC 0.8309 · 14 pathologies</h4>"
    )
